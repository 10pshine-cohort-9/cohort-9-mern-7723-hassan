...
jest.mock("../components/AvailableNotes", () => ({
  __esModule: true,
  default: ({ filesList, onOpenFile, currentFileId }) => (
...
describe("Sidebar", () => {
  ...
  beforeEach(() => {
    ...
    axios.get.mockImplementation((url) => {
      ...
      if (url.includes("/note/507f1f77bcf86cd799439011")) {
        return Promise.resolve({
          data: {
            success: true,
            file: {
  ...
  test("renders sidebar buttons", async () => {
    try {
      ...
      throw new Error("Sidebar: renders sidebar buttons failed", {
        cause: error,
      });
    }
  });
  ...
  test("calls onNew when Create New is clicked", async () => {
    try {
      ...
      throw new Error(
        "Sidebar: calls onNew when Create New is clicked failed",
        { cause: error }
      );
    }
  });
  test("calls onSave when Save is clicked", async () => {
    try {
      ...
      throw new Error(
        "Sidebar: calls onSave when Save is clicked failed",
        { cause: error }
      );
    }
  });
  test("calls onExport when Export is clicked", async () => {
    try {
      ...
      throw new Error(
        "Sidebar: calls onExport when Export is clicked failed",
        { cause: error }
      );
    }
  });
  ...
  test("shows available notes after clicking Open", async () => {
    try {
      ...
      expect(screen.getByTestId("available-notes")).toBeInTheDocument();
